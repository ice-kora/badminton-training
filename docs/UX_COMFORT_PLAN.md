...
## Amateur review follow-ups（鸡肋面降权 / 示范动图）

### 产品红黑名单（决策）

| 面 | 决策 | 说明 |
|----|------|------|
| 3D 棍人（V3） | **黑名单主路径** / 灰名单次要入口 | 不删代码；技能详情降为「实验演示」，诚实提示真人教练视频体感更好 |
| 7 日计划打卡卡 | **灰名单** | 训练 Tab 主路径改为「今日优先练 / 去拍摄」；日卡折叠，打卡标「可选记录」 |
| 毫秒 Δt 时间轴 | **黑名单（业余主 UI）** | API 仍可保留 `delta_ms`；界面用阶段名（引拍/挥拍/击球/随挥）+ 有意义时「偏慢/偏快」 |
| 骨架叠加 | **灰名单（默认关）** | 并排对比为主；骨架/叠加 opt-in |
| Drill 示意动图 | **白名单** | `demo_media_url` / `demo_gif_url`；示意 GIF 标明「非真人教练」 |
| 音频甜区 / 30s→3s 高光 / 假分实时 | **黑名单** | 不做、不假装 |

### 验收补记

- 技能详情主 CTA = 拍摄；3D 仅次要「实验演示」
- 训练 Tab 不把 Day1–Day7 作为首屏主导
- 结果/详情不展示 `Δt=+85ms` 实验室毫秒文案
- 骨架默认关；Drill 详情/结果 CTA 有循环示意或短「看示范」占位
